...
// #include "pid.h"
// #include "num.h"
// #include "position_controller_Clover.h"
...
//   struct pidInit_s init;       // kp, ki, kd
//     stab_mode_t previousMode;  // previous mode
//   float setpoint;              // setpoint x, y, z

//   float output;                // output
// };

// struct this_s {
//   struct pidAxis_s pidVX;      // gain, previous mode, setpoint, output
...
//   uint16_t thrustBase;        // approximate throttle need when in perfect hover. More weight/older battery can use higher value
...
// void positionControllerCloverInit()
// {
...
//   axis->setpoint = setpoint; // set setpoint to pid struct

//   pidSetDesired(&axis->pid, axis->setpoint);  // get desired setpoint
//   return pidUpdate(&axis->pid, input, true);  // calcurate pid controller output
// }

// void positionControllerClover(float* thrust, attitude_t *attitude, setpoint_t *setpoint,
//                                                              const state_t *state)
// {
//   this.pidX.pid.outputLimit = xyVelMax * velMaxOverhead; //input velocity limit
//   this.pidY.pid.outputLimit = xyVelMax * velMaxOverhead;


//   this.pidZ.pid.outputLimit = fmaxf(zVelMax, 0.5f) * velMaxOverhead; //saturation with 0.5
...
//   // Vx command or calcurate from position error
...
//   }
//   // Vy command or calcurate from position error
...
//   }
//   // Vz camannd only in position controller
//   if (setpoint->mode.z == modeAbs) {
//     setpoint->velocity.z = runPid(state->position.z, &this.pidZ, setpoint->position.z, DT);
//   }

//   velocityControllerClover(thrust, attitude, setpoint, state);
// }

// void velocityControllerClover(float* thrust, attitude_t *attitude, setpoint_t *setpoint,
//                                                                const state_t *state)
// {
//   this.pidVX.pid.outputLimit = rpLimit * rpLimitOverhead; // roll and pitch angle permited
//   this.pidVY.pid.outputLimit = rpLimit * rpLimitOverhead; // roll and pitch angle permited
//   // set the output limit to the maximum thrust range
//   this.pidVZ.pid.outputLimit = (40000 / 2 / thrustScale); // 2^15 / thrustscale(1000) ~ 32
...
//   float yawRad = state->attitude.yaw * (float)M_PI / 180; // degree to radian
//   attitude->pitch = -(rollRaw * cosf(yawRad)) - (pitchRaw * sinf(yawRad)); // body roll 
//   attitude->pitch = -(pitchRaw * cosf(yawRad)) - (rollRaw * sinf(yawRad)); // body pitch
...
//   // Check the minimum thrust
//   if(*thrust < this.thrustMin) {
//     *thrust = this.thrustMin;
//   }
// }

// void positionControllerCloverResetAllPID()
// {
...
// LOG_GROUP_STOP(posCtlClover)

// PARAM_GROUP_START(velCtlCloverPid)
...
// PARAM_GROUP_STOP(velCtlCloverPid)

// PARAM_GROUP_START(posCtlCloverPid)
...
// PARAM_GROUP_STOP(posCtlCloverPid)
